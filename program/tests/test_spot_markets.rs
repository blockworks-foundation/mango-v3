...
async fn test_place_spot_order() {
    ...
    let expected_values: HashMap<&str, I80F48> =
        [("quote_locked", I80F48::from_num(10000000000 as i64))].iter().cloned().collect();
...
async fn test_match_spot_order() {
    ...
    let (bidder_quote_free, bidder_quote_locked, bidder_base_free, bidder_base_locked) =
        test.get_oo_info(&mango_group_cookie, bidder_user_index, mint_index).await;
    ...
    let (asker_quote_free, asker_quote_locked, asker_base_free, asker_base_locked) =
        test.get_oo_info(&mango_group_cookie, asker_user_index, mint_index).await;
